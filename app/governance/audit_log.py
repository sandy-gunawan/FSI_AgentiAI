"""Persistent audit log — every agent step, tool call, and decision.

Stored in SQLite locally (data/audit.db); the same interface maps to Azure
Database for PostgreSQL in the cloud. This is the governance backbone: the
Streamlit "Audit & Governance" page reads from here.
"""
...
class AuditLogger:
    ...
    def record(
        ...
        """Convenience wrapper that builds and stores an AuditEvent."""
        self.log(
            AuditEvent(
    ...
    def events_for(self, request_id: str) -> list[dict]:
        cur = self._conn.execute(
            ...
            "FROM audit_events WHERE request_id=? ORDER BY id",
            (request_id,),
        )
