...
It *reasons* over the input and produces the output. In this demo we use **two agents**:
...
## What "2 options" means

The only thing that changes between the two options is **how Agent 1 sees the invoice**:

- **Option A** — Azure **Document Intelligence** does the OCR first (deterministic,
  gives confidence scores), then Agent 1 tidies it up.
- **Option B** — a **multimodal** model (that can *see* images) reads the picture directly.

Agent 2 and the rules engine are identical in both. See [03](03-the-two-options.md).
